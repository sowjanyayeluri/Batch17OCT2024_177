 ...
 void add_details(long int snumber,long int rnumber,char *type)
{
 ...
 int is_rphno_exist(long int sphno,long int rphno)
 {
     pthread_mutex_lock(&file_mutex);
        	...
        	if (strcmp(password, confPassword) == 0)
         	{
				...
				log_changes(DEBUG,"Registered successfully\n");
...
void unregister_client(int client_sock)
{
 ...
 void forward_call(int client_sock)
 {
     long int rphno;
     recv(client_sock,&rphno,sizeof(rphno),0);
     if(is_rphno_exist(sphno,rphno)==2)
     {
...
6         }
     ...
     else if(is_rphno_exist(sphno,rphno)==1)
     {
         printf("%ld is not a valid number\n",rphno);
     }
     else
 ...
 void log_changes(LogLevel level, const char *function_name) {
    ...
    logfile = fopen("logpractice.txt", "a");
